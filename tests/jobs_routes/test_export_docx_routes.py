...
def test_post_returns_200_on_regeneration_and_bumps_count(
    ...
    See the matching PDF test for the rationale. The DOCX renderer is
    likewise byte-deterministic, so a re-render against the same
    markdown is an idempotent no-op (regeneration_count stays at 0;
    regenerated is False). Mutating the markdown (and refreshing the
    manifest's recorded sha) is what produces the actual lineage
    append.
    """
    job_id = approved_job_ready_for_export
